...
set val(x)		50
set val(y)		50
set val(nam)		wpan_Proyecto.nam    ;#se crea un archivo .nam que va a correr para la simulacion
...
# se hace la configuracion del nodo y se carga con los valores antes inicializados, que estan en el principio del codigo
...
# con esta parte del codigo se desactiva el movimiento aleatorio
...
##aqui se define la posicion inicial de los nodos tomando en cuenta los nodos
$node_(0) set X_ 25.0
$node_(0) set Y_ 25.0
$node_(0) set Z_ 0.0
$node_(1) set X_ 0.0
...
$ns_ at 0.0	"$node_(7) sscs startPANCoord"		
...
$ns_ at 0.5	"$node_(0) sscs startDevice 1 0"	
$ns_ at 1.5	"$node_(2) sscs startDevice 2 0"
$ns_ at 2.5	"$node_(4) sscs startDevice 3 0"
$ns_ at 3.5	"$node_(6) sscs startDevice 4 0"

#en el tiempo 1 que se define se va a comenzar con la transmision de las tramas de beacon
Mac/802_15_4 wpanNam PlaybackRate 3ms
$ns_ at $appTime1 "puts \"\nTransmitting data ...\n\""

# Configuracion del flujo de trafico entre los nodos, en esta parte van a ir el tipo de mensaje y que protocolo de transporte se va a utilizar para cada mensaje en este caso es udp, ademas se define el tipo de paquete, su tamanio, un control en el flujo en este caso cbr(constant bit rate), el maximo numero de paquetes 
...
# Configuracion del flujo de trafico entre los nodos basado en una distribucion de possion esto es en base a una probabilidad que tenga cada nodo en transmitir, en esta parte van a ir el tipo de mensaje y que protocolo de transporte se va a utilizar para cada mensaje en este caso es udp, ademas se define el tipo de paquete, su tamanio, un control en el flujo en este caso cbr(constant bit rate), el maximo numero de paquetes, ademas del intervalo en el que se transmite los paquetes
...
#en esta parte del codigo se define una condicion dependiendo del tipo de trafico que se va a manejar de tal manera que se manda a realizar un conjunto de funciones dependiendo si es trafico con CBR o POISSON, se determina el sentido de los flujos de tramas y en que tiempo estas deben ponerse en marcha, y se imprime los resultados para que aparezca en la interfaz grafica. tambien se configura los para diferenciar el tipo de paquetes
...
# Configuracion del flujo de trafico entre los nodos con ftp, en esta parte van a ir el tipo de mensaje y que protocolo de transporte se va a utilizar para cada mensaje en este caso es ftp, ademas se define el tipo de paquete, su tamanio, un control en el flujo en este caso cbr(constant bit rate), el maximo numero de paquetes
...
#en esta parte del codigo se define una condicion dependiendo del tipo de trafico que se va a manejar de tal manera que se manda a realizar un conjunto de funciones para el protocolo de transporte ftp, se determina el sentido de los flujos de tramas y en que tiempo estas deben ponerse en marcha, y se imprime los resultados para que aparezca en la interfaz grafica. tambien se configura los para diferenciar el tipo de paquetes   
if { "$val(traffic)" == "ftp" } {
   puts "\nTraffic: ftp"
...
# define la posicion de los nodos que en este caso va a ser con una topologia estrella pero ya esta definida mas arriba para que sea lineal
for {set i 0} {$i < $val(nn)} {incr i} {
	$ns_ initial_node_pos $node_($i) 2
}

#se reseta los nodos cuando se detiene o termina la simulacion 
for {set i 0} {$i < $val(nn) } {incr i} {
...
# se hace un procedimiento de detencion de la simulacion, en donde se cierra el archivo .nam que se ejecuto anteriormente 
proc stop {} {
...
#se pone en marcha o a correr la simulacion
puts "\nStarting Simulation..."
$ns_ run
